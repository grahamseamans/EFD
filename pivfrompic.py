...
lems = os.path.join(os.getcwd(), "data", "alllems")
imdir = os.path.join(os.getcwd(), "data", "images")


video = np.load(imdir + "/time_svded_video.npy")
num_frames, frame_width, frame_height = video.shape
video = video.reshape(num_frames, frame_width * frame_height)
...
np.save(lems + "/sdf.npy", sdf)
np.save(lems + "/ddf.npy", ddf)
...
"""
sonicdir = ddf.mean(axis=1)
(guess_dir + 65) % 360  # reorients vector into real space
...
X_train, X_test, y_train, y_test = train_test_split(
    video, sonicdir, test_size=0.2, random_state=0
)
